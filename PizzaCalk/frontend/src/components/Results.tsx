      ...
      <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
        ...
        {pricePerSlice > 0 && (
          <div className="mt-4 text-center">
            <div className="text-gray-600 text-sm mb-1">Цена одного куска</div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-wrap gap-1">
                  {Array.from({ length: userSlicesDistribution[user.id] || 0 }).map((_, i) => (
                    <span key={i} className="text-lg" title="Кусок пиццы">🍕</span>
                  ))}
                </div>
              </div>
      ...
      {commonSlices > 0 && (
        <div className="bg-green-50 border-2 border-green-200 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <span className="font-medium text-green-900">Общие куски</span>
              <div className="flex flex-wrap gap-1">
                {Array.from({ length: commonSlices }).map((_, i) => (
                  <span key={i} className="text-lg" title="Общий кусок">🍕</span>
                ))}
              </div>
            </div>
          ...
          >
            {splitCommonSlices ? '✓ Разделено на всех' : 'Разделить на всех'}
          </button>
        </div>
      )}
